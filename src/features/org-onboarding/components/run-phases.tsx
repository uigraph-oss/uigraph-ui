...
const PHASES = [
  {
    ...
    summary:
      'Opening a branch of its own, so your default branch is left alone.',
    failureHint:
      'The branch could not be created. Check that UIGraph is still connected to this repository, then retry.',
  },
  {
    label: 'Preparing',
    icon: LuServer,
    summary: 'Starting a fresh machine and putting your repository on it.',
    failureHint:
      'The run could not start. Check that GitHub Actions is enabled for this repository, then retry.',
  },
  {
    label: 'Installing',
    icon: LuPackage,
    summary: 'Putting the tools that read your code in place.',
    failureHint:
  ...
  },
  {
    ...
    icon: LuScanSearch,
    summary:
      'Reading your pages, components, and API calls. This is the longest part.',
    failureHint:
      'Your AI provider turned the request down. Check the provider settings on this repository, then retry.',
  },
  {
    label: 'Wrapping Up',
    icon: LuGitCommitVertical,
    summary: 'Saving what it found back to your repository.',
    failureHint:
      'The results could not be saved back to your repository. Check that UIGraph is allowed to write to it, then retry.',
  },
  {
    label: 'Publishing',
    icon: LuCloudUpload,
    summary: 'Sending the graph over and putting the pieces together.',
    failureHint:
  ...
  },
  {
    label: 'Finishing Up',
    icon: LuFlag,
    summary: 'Running the last checks before your graph opens.',
    failureHint:
...
export function runPhases(value: RunImport | null): RunPhase[] {
  ...
  if (value) {
    tracked[0].startedAt = value.createdAt
    tracked[0].completedAt = value.runStartedAt ?? null
  }
  ...
  const failedIndex = tracked.findIndex((phase) => phase.failed)
...
export function currentRunPhase(phases: RunPhase[]) {
  const index = phases.findIndex(
    (phase) => phase.status === 'failed' || phase.status === 'active'
  )
  if (index === -1) return phases.length - 1
  return index
}
  ...
  return (
    <ol className="flex h-full flex-col">
      {phases.map((phase, index) => (
        <li
          key={phase.label}
          className="relative flex flex-1 items-center gap-3.5"
        >
