...
import { z } from 'zod'
import { DynamoAttributeSchema } from './nosql-schema'

type NestedFieldEditorProps = {
  isFirstLevel?: boolean
  fields: Array<z.infer<typeof DynamoAttributeSchema>>
  parentLabel: string
  onChange: (fields: Array<z.infer<typeof DynamoAttributeSchema>>) => void
  normalizeField: (
    field: z.infer<typeof DynamoAttributeSchema>,
    patch: Partial<z.infer<typeof DynamoAttributeSchema>>
  ) => z.infer<typeof DynamoAttributeSchema>
  expanded: Set<string>
...
export function DynamoRecursiveAttributeCard({
  isFirstLevel,
  fields,
  parentLabel,
  onChange,
  ...
  function addField() {
    onChange([
      ...fields,
      { id: generateUUID(), name: '', type: '', required: false },
    ])
  }
  ...
  function setChildFields(
    ...
    nextFields: Array<z.infer<typeof DynamoAttributeSchema>>
  ) {
    onChange(
      fields.map((field) =>
                ...
                <div key={field.id} className="space-y-2">
                  <div className="grid grid-cols-[2fr_1.2fr_auto_auto] items-center gap-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            updateField(
                              ...
                              value === 'object'
                                ? { itemType: value }
                                : { itemType: value, itemFields: undefined }
                            )
                          }
                        >
                      ...
                      {allowItemFields && isArrayOfObjects && (
                        <button
                          type="button"
                          onClick={() => onToggle(field.id)}
                          className="flex items-center gap-1 rounded-md border border-transparent p-2 text-gray-600 transition-colors hover:bg-gray-50"
